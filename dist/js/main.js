(function () {
  ...
  function getTransitionDuration(element) {
    ...
    const duration = computed.transitionDuration.split(',')[0];
    const delay = computed.transitionDelay.split(',')[0];
    const total = parseFloat(duration) + parseFloat(delay);
    if (!total) {
      return 0;
    }
    return total * 1000;
    ...
    buttons.forEach(button => {
      button.onclick = () => {
        ...
        if (currentActiveButton) {
          currentActiveButton.classList.remove(ACTIVE_CLASS$3);
        }
        const newSrc = `${originalSrc}&parentID=${parent}`;
        const {
          form: newForm
        } = createIframe(template, newSrc);
        button.classList.add(ACTIVE_CLASS$3);
  ...
  navElements.forEach(nav => {
    ...
    subnavToggles.forEach(toggle => {
      toggle.addEventListener('click', () => {
        toggle.parentNode.classList.toggle(ACTIVE_CLASS$2);
        const isOpen = nav.querySelector(`.${ACTIVE_CLASS$2}`);
    ...
    function clickOutside(event) {
      if (!nav.contains(event.target)) {
        nav.querySelectorAll('.has-subnav').forEach(item => {
          item.classList.remove(ACTIVE_CLASS$2);
        });
  ...
  const buttons$1 = document.querySelectorAll('[data-modal]');
  ...
  function hideModal(target) {
    ...
    target.classList.remove(ACTIVE_CLASS$1);
    setTimeout(() => {
  ...
  function showModal(target) {
    ...
    target.classList.add(ACTIVE_CLASS$1);
  ...
  const Default = {
    ...
    maxResults: 3,
    shuffleLevel: 5,
    imageSize: 'w30-h30-p-k-no-nu-rw-l80-e30',
  ...
  function createEntry(entry) {
    ...
    $item.className = 'related-item flex g-1 has-image items-center relative';
    $imgContainer.className = 'related-image ratio ratio-1:1 color-text flex flex-none items-center justify-center i-s';
